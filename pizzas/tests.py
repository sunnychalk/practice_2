...
class PizzaModelTestCase(TestCase):
    ...
    def setUp(self):
        Pizza.objects.create(
            name='New pizza', price=10.0, description="new pizza",
            photo="static/pizzas/no_photo.png"
        )
...
class ShippingFormTest(TestCase):
    def setUp(self):
        Shipping.objects.create(
            email="user@user.com", first_name="user", last_name="user2",
            street="street", city="Odessa", postcode="000000",
            region="Odessa region", country="Ukraine"
        )

    def test_shipping_form_valid(self):
        form = ShippingForm(
            data={
                "email": "user@user.com", "first_name": "user",
                "last_name": "user2", "street": "street",
                "city": "Odessa", "postcode": "000000",
                "region": "Odessa region", "country": "Ukraine"
            }
        )
    ...
    def test_shipping_form_invalid(self):
        form = ShippingForm(
            data={
                "email": "ufgth.com", "first_name": "gh", "last_name": "h",
                "street": "s", "city": "ghj", "postcode": "87",
                "region": "egion", "country": "Ue"
            }
        )
